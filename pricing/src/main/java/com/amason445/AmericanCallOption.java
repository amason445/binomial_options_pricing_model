...
public class AmericanCallOption {
    ...
    public AmericanCallOption(int N, double s0, double K, double r, double sigma, double T) {
        this.N = N;
        this.s0 = s0;
        this.K = K;
        this.r = r;
    ...
    public double calculateOptionPremium() {
        ...
        for (int i = 0; i <= N; i++) {
            ...
            option_values[i][N] = Math.max(0, stockPriceAtMaturity - K);
        }
        ...
        for (int j = N - 1; j >= 0; j--) {
            for (int i = 0; i <= j; i++) {
                ...
                double exerciseValue = Math.max(0, stockPriceAtCurrentNode - K);
