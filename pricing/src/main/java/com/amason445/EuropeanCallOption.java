...
public class EuropeanCallOption {
    ...
    public EuropeanCallOption(int N, double s0, double K, double r, double sigma, double T) {
        this.N = N;
        this.s0 = s0;
        this.K = K;
        this.r = r;
    ...
    public double calculateOptionPremium() {
        ...
        for (int j = N - 1; j >= 0; j--) {
            for (int i = 0; i <= j; i++) {
                option_values[i][j] =  discountFactor * (p * option_values[i][j + 1] + (1 - p) * option_values[i + 1][j + 1]);
                }
            }
