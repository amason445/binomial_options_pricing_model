...
In these models, the present value is calculated using a binomial value where "p" is a risk neutral calculated using the risk free rate, the upswing and downswing values. This process is similar to calculating a weighted average of the prior two nodes with p and (1-p). Once this is complete, this average is then discounted and the final node selected is (0,0) which contains the fair value of the option today.
